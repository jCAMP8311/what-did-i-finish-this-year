...
export const GameBar: FC<GameBarProps> = ({ cover, title, platform, completion, rating, review }) => {
	...
	return (
		...
		<Box pb={'5px'} pl={'5px'} w='100%'>
			<Heading fontFamily={'fantasy'} lineHeight='short' pt={'8px'} size={'xs'} >
				{/* TODO: This deterministic styling is gross. We can make it better. */}
  				<Highlight query={title} styles={ platform === 'xbox' ? { px: '2', py: '1', rounded: 'full', bg: 'green.100' } : { px: '2', py: '1', rounded: 'full', bg: 'blue.100' }} >
			  		{title}
  				</Highlight>
			</Heading>
		</Box>
		<HStack h='100px' p={5}>
				...
				<Box alignContent={'center'} boxSize={"75px"} display={'grid'} p={'5px'} >
					<Image alt='Platform logo' boxSize='60px' src={`./images/${platform}.png`} />
				</Box>
				<Divider borderColor={'blackAlpha.300'} orientation='vertical' />
				...
				<Divider borderColor={'blackAlpha.300'} orientation='vertical' />
				<Box boxSize={"75px"}>
					<Image src={`./images/${completionPlatform}.png`} alt='Platform completion' />
